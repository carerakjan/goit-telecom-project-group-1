...
def render_single_tab():
    ...
    with st.form(key=f'single_prediction_form_{st.session_state.user_count}'):
        ...
        subscription_age = st.number_input('Вік підписки (в місяцях)', min_value=0.0, value=0.0)
        reamining_contract = st.number_input('Залишок контракту (в місяцях)', min_value=0.0, value=0.0)
    ...
    if submit_button:
        data = {
            'is_tv_subscriber': 1.0 if is_tv_subscriber == 'Так' else 0.0,
            'is_movie_package_subscriber': 1.0 if is_movie_package_subscriber == 'Так' else 0.0, 
